...
from arduino_magnetic_sensor import LoadTactileData
...
def getCompleteFilePaths():
    ...
    rospy.loginfo("Opening Tactile data file: {}".format(config_dict['tactile_data_path']+"Grasp_Experiment_"+config_dict['experiment']['object']+"_"+
        config_dict['experiment']['state']+'_'+config_dict['experiment']['sensor_prototype']+'_'+str(config_dict['experiment']['object_num'])+'_'))
    config_dict['tactile_data_path'] = glob.glob(config_dict['tactile_data_path']+"Grasp_Experiment_"+config_dict['experiment']['object']+"_"+
        config_dict['experiment']['state']+'_'+config_dict['experiment']['sensor_prototype']+'_'+str(config_dict['experiment']['object_num'])+'_' + "*.bag")[0]
    rospy.loginfo("Opening Tactile data file: {}".format(
        config_dict['tactile_data_path']))

    config_dict['experiment_key'] = config_dict['experiment']['sensor_prototype'] + \
        '/' + config_dict['experiment']['object'] + '_' + str(config_dict['experiment']['object_num']) + \
        '/' + config_dict['experiment']['state'] + \
...
def readConfigFile():
    ...
    if rospy.has_param('/generate_h5py/json_file_name'):

        config_file_path = rospy.get_param(
            "/generate_h5py/json_file_name")
...
def saveH5pyFile(experiment_key, file_path, tactile):
    ...
    group_tactile.create_dataset(
        'tactile_data', data=tactile.np_sensor_readings[tactile_start_index:tactile_end_index, :, :])
    group_tactile.create_dataset(
        'tactile_data_calibrated', data=tactile.np_sensor_readings_calibrated[tactile_start_index:tactile_end_index, :, :])
    group_tactile.create_dataset(
        'tactile_data_dispersion', data=tactile.np_sensor_dispersion[tactile_start_index:tactile_end_index])
# group_tactile.create_dataset(
...
def on_press(key):
    ...
    try:
        ...
        if hasattr(key, 'char') and key.char == "h":
            ...
            rospy.loginfo('You entered a number from the numpad: {}'.format(str(key.char)))
            if key.vk == 48:
                # exp_number = int(raw_input('Enter number: '))
                config_dict['experiment']['experiment_num'] +=1
                if config_dict['experiment']['experiment_num'] == 20:
                    config_dict['experiment']['experiment_num'] = 0
            else:
                config_dict['experiment']['experiment_num'] = int(key.char)

            rospy.loginfo('Changing to record Experiment: {}'.format(str(key.char)))
            
            # config_dict['experiment']['experiment_num'] = exp_number
...
if __name__ == '__main__':
    ...
    rospy.init_node('generate_h5py', anonymous=True)
    ...
    while not rospy.is_shutdown():
        ...
        for i, timestamp in enumerate(tactile.timestamps):
            try:
                if tactile.timestamps[i+1]- tactile.timestamps[i] > 1.0:
                    tactile.setTimestampEndExperiment()
                    saveH5pyFile(config_dict['experiment_key'],
                                config_dict['h5py_file_path'],
                                tactile)
                    tactile.updateNextIndex()
                    tactile.setTimestampBeginningExperiment()
        ...
        rospy.loginfo("Saving data for Object {} and Experiment {}".format(config_dict['experiment']['object'] + '_' + str(config_dict['experiment']['object_num']), str(config_dict['experiment']['experiment_num'])))
        rospy.loginfo('Current sensor contact dispersion: {}'.format(
            tactile.np_sensor_dispersion[tactile.current_index]))
